...
import type {
  ...
  LorebookEntry as PrismaLorebookEntry,
} from "@prisma/client";

export type Character = PrismaCharacter;
export type LorebookEntry = PrismaLorebookEntry;
...
class InMemoryDB {
  ...
  async getCharacters(
    projectId: string,
  ): Promise<Result<{ characters: Character[] }>> {
    const characters = Array.from(this.characters.values()).filter(
  ...
  async getLoreEntries(
    ...
    let entries = Array.from(this.lorebookEntries.values()).filter(
...
export class ContextBuilder {
  ...
  async buildCharacterContext(
    ...
    const searchTerms = [
      ...keywords,
      ...(context.mainCharacter
        ? [context.mainCharacter.name.toLowerCase()]
        : []),
    ];
    ...
    for (const term of searchTerms) {
      ...
      if (r.success)
        r.entries.forEach((e) =>
          loreScores.set(e.id, (loreScores.get(e.id) || 0) + e.priority + 1),
        );
    }
  ...
  async buildStoryContext(userPrompt: string) {
    ...
    if (lore.success) {
      ...
      context.worldSummaries = Object.entries(byCategory).map(
        ([cat, entries]) => {
